...
typedef struct saveconf_header_s {
        char magic_str[16];
#define SAVE_MAGIC_STRING "raw_nv"
        int zip_img_size;
        int crc;
        int max_size;
        int fsid;
...
typedef struct sys_params_s {
      ...
      unsigned char lanmac[32];
      unsigned char wanmac[32];
      int diag;
      int ated;
//#ifdef USE_WPS_PINCODE_FLASH
      char pincode[16];
//#endif
//#ifdef USE_RT388X
      int flag;
#define SUPPORT_RTL8367B_FLAG 0x1  /* first bit 0: RTL8370, first bit 1 : RTL8367B */
//#endif
} sys_params_t;
...
int flash_get_sysparams( sys_params_t *params)
{
        ra_mtd_read_nm("Bootloader", BOOT_SIZE-SYSPARMS_MINUS_OFFSET, sizeof(sys_params_t),params);
        return 1;
}

int check_diag_flag(void)
{
        sys_params_t params;

	memset(&params,0x0,sizeof(sys_params_t));
        if(!flash_get_sysparams(&params))
                return 0;

        return params.diag;
}

#define BOOTCONFIG_MINUS_OFFSET   0x200
...
#else
        unsigned int reserved[121];
#endif
} boot_config_t;

int flash_get_bootconfig( boot_config_t *bc)
{
        ra_mtd_read_nm("Bootloader", BOOT_SIZE-BOOTCONFIG_MINUS_OFFSET, sizeof(boot_config_t),bc);
        if(!strncmp(bc->boot_magic,BOOT_CONF_MAGIC_VAL, strlen(BOOT_CONF_MAGIC_VAL)))
                return 1;
        return 0;
}

int check_ftm_flag(void)
{
        boot_config_t bc;

        if(!flash_get_bootconfig(&bc)) return 0;

        if(!strncmp(bc.factory_test_mode,FACTORY_TEST_MODE_MAGIC,strlen(FACTORY_TEST_MODE_MAGIC)))
		return 1;
	return 0;
}
...
int xhci_u2_only=0;
int get_usb20_only(void)
{
	saveconf_header_t sc;
        ra_mtd_read_nm("Config", 0, sizeof(saveconf_header_t),&sc);


	if(check_diag_flag())
	{
		printk("######## Test Processs is ON. USB3.0 will be used\n");
		return 0;
	}

	if(check_ftm_flag())
	{
		printk("######## Factory Test Mode is ON is ON. USB3.0 will be used\n");
		return 0;
	}


        if(strncmp(sc.magic_str,SAVE_MAGIC_STRING, strlen(SAVE_MAGIC_STRING)))
	{
		return 1; /* Default is USB2.0 only */
	}	

	if(sc.flag & USB30_ENABLE_FLAG)
		return 0;
	return 1;
}
#endif
...
#ifdef CONFIG_EFM_PATCH
	xhci_u2_only=get_usb20_only();
	printk("------------> USB3.0 is %s\n", xhci_u2_only?"Disabled":"Enabled");
#endif
