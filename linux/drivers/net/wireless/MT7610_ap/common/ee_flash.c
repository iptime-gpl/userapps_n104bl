...
/* ov str :   offset:value,offset2:value,... */
int OverridingEEPROM(UCHAR *eeprom,CHAR *ovstr)
{
        char *p,*p2;
        char buf[MAX_OVERRIDING_EEPROM];
        int offset,val;

        printk("OverridingEEPROM ----> %s\n", ovstr);

        strncpy(buf,ovstr,MAX_OVERRIDING_EEPROM-1);
        ...
        for(p=buf;p;)
        {
                ...
                if(!p2) break;
                *p2=0;
                sscanf(p,"%X:%X",&offset,&val);
                printk("OFFSET:%x VAL:%x\n",offset,val);
                if(offset > MAX_EEPROM_BUFFER_SIZE)
                        continue;
                eeprom[offset]=val&0xff;
                eeprom[offset+1]=(val>>8)&0xff;
                p=p2+1;
        }
        return 1;
...
#endif
#ifdef EFM_MISC_PATCH
        if(strlen(pAd->CommonCfg.OverridingEEPROM))
        {
                OverridingEEPROM(pAd->chipCap.eebuf,pAd->CommonCfg.OverridingEEPROM);
        }
#endif
