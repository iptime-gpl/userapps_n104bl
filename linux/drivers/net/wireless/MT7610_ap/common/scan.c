	...
	if (OpMode == OPMODE_AP)
	{
			...
			(pAd->ApCfg.bImprovedScan))
		{
			...
			pAd->ApCfg.bImprovedScan = FALSE;
                ...
                BOOLEAN ScanPending = ((pAd->ApCfg.bImprovedScan) && (pAd->ApCfg.ScanChannelCnt>=IMPROV_SCAN_CH_COUNT));

                        if ((pAd->ApCfg.ApCliAutoConnectRunning == TRUE) && (ScanPending == FALSE))
                        {
...
static INT scan_active(RTMP_ADAPTER *pAd, UCHAR OpMode, UCHAR ScanType)
{
	...
	SsidLen = 0;
#if !defined (APCLI_CONNECTION_TRIAL) || defined (APCLI_AUTO_CONNECT_SUPPORT)
...
#ifdef CONFIG_AP_SUPPORT
	ScanPending = ((pAd->ApCfg.bImprovedScan) && (pAd->ApCfg.ScanChannelCnt>=IMPROV_SCAN_CH_COUNT));//7
#endif
		...
		if (SCAN_MODE_ACT(ScanType))
		{
			...
			if (needUnicastScan)
			{
			    ...
			    for(index = 0; index < MAX_APCLI_NUM; index++)
				{
					...
					if (desiredSsidLen  > 0)
					{
						DBGPRINT(RT_DEBUG_OFF, ("%s:: specific the %s scanning\n", __FUNCTION__, 
						...
						NdisCopyMemory(pAd->MlmeAux.Ssid, pAd->ApCfg.ApCliTab[index].CfgSsid, desiredSsidLen);
