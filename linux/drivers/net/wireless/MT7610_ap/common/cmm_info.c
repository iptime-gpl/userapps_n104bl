        ...
        IN      RTMP_IOCTL_INPUT_STRUCT  *wrq)
{
       ...
       /* sprintf(msg,"%d\n",ApScanRunning(pAdapter)); */
        sprintf(msg,"%d\n", ScanRunning(pAdapter)); /* 0:AP_SYNC_IDLE , 1: AP_SCAN_LISTEN, 2: AP_SCAN_PENDING */
        wrq->u.data.length = strlen(msg);
...
int Get_HTPhy_Rate( HTTRANSMIT_SETTING rate, char *str)
{
        ...
        sprintf(str, "%lu", rateval);
        return 0;
}
#ifdef EFM_MISC_PATCH_XXX
VOID RTMPIoctlGetApcliStatus(
        IN      VOID *pAdapter,
        IN      RTMP_IOCTL_INPUT_STRUCT  *wrq)
{
        ...
        if (pObj->ioctl_if_type != INT_APCLI)
        {
                printk("Not APCLI interface\n");
                return;
        }

        ifIndex = pObj->ioctl_if;

        os_alloc_mem(NULL, (PUCHAR *)&msg, 64);
        if (msg == NULL)
        {
                return;
        }
                ...
                for (i=0; i<MAX_LEN_OF_MAC_TABLE; i++)
                {
                        ...
                        if ( IS_ENTRY_APCLI(pEntry)
                                ...
                                && (tr_entry->PortSecured == WPA_802_1X_PORT_SECURED))
                                {
                                        ...
                                        printk("Connected -- %s : WpaState:%d\n",  msg, pEntry->WpaState);
                                        break;
                                }
                                else
                                        sprintf(msg,"Connecting,%d,Sst:%d,PortSecured:%d", RTMPAvgRssi(pAd,&pEntry->RssiSample), pEntry->Sst, tr_entry->PortSecured);
                }
        ...
        wrq->u.data.length = strlen(msg);
        copy_to_user(wrq->u.data.pointer, msg, wrq->u.data.length);
        printk("wrq->u.data.pointer->msg:%s length:%d\n", msg,wrq->u.data.length);
        os_free_mem(NULL, (PUCHAR)msg);
}
#endif
#endif
