...
#ifdef EFM_MISC_PATCH
/* for WPAPSKWPA2PSK_APCLI_PATCH */
                        pAd->ApCfg.ApCliTab[i].bMixAuthMode = FALSE;
#endif
...
#ifdef EFM_MISC_PATCH
/* for WPAPSKWPA2PSK_APCLI_PATCH */
                        else if (rtstrcasecmp(tmpbuf, "WPAPSKWPA2PSK") == TRUE)
                        {
                                pApCliEntry->AuthMode = Ndis802_11AuthModeWPA1PSKWPA2PSK;
                                pAd->ApCfg.ApCliTab[i].bMixAuthMode = TRUE;
                        }
#endif
			else
...
#ifdef  EFM_MISC_PATCH
                /*StrictRegulationForKR*/
                if(RTMPGetKeyParameter("StrictRegulationForKR", tmpbuf, 25, pBuffer, TRUE))
                {
                        pAd->CommonCfg.StrictRegulationForKR = simple_strtol(tmpbuf, 0, 10);
                        retval = 0;
                }

                if(RTMPGetKeyParameter("EEPROM_OVERRIDE", tmpbuf, MAX_OVERRIDING_EEPROM-1, pBuffer, TRUE))
                {
                        strncpy(pAd->CommonCfg.OverridingEEPROM,tmpbuf,MAX_OVERRIDING_EEPROM-1);
                        retval = 0;
                }
                else
                {
                        printk("CLEAR EEPROM_OVERRIDE\n");
                        memset(pAd->CommonCfg.OverridingEEPROM,0x0,MAX_OVERRIDING_EEPROM);
                }


                /*EFM_LedMode*/
                if(RTMPGetKeyParameter("EFM_LedMode", tmpbuf, 25, pBuffer, TRUE))
                {
                        pAd->CommonCfg.EFM_LedMode = simple_strtol(tmpbuf, 0, 10);
                        retval = 0;
                }
#endif
