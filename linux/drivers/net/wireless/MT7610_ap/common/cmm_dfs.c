	...
	IN PRTMP_ADAPTER pAd)
{
	...
	for (i=0; i<pAd->ChannelListNum; i++)
	{		
		pAd->ChannelList_BackUp[i].RemainingTimeForUse = pAd->ChannelList[i].RemainingTimeForUse;
		pAd->ChannelList_BackUp[i].Channel = pAd->ChannelList[i].Channel;	
	}
 	...
 	IN PRTMP_ADAPTER pAd)
{
	...
	if (IS_MT7610E(pAd))
	{
		...
		{
			DfsDetectionDisable(pAd);
			RTMP_BBP_IO_WRITE32(pAd, TR_R0, 0);
			DfsDetectionEnable(pAd);
#ifdef CARRIER_DETECTION_SUPPORT
			if (pAd->CommonCfg.CarrierDetect.Enable == TRUE) {
				RTMP_BBP_IO_WRITE32(pAd, TR_R0, 1);
			}
