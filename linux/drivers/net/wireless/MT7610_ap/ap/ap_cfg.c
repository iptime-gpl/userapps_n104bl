	...
	IN	PSTRING			arg)
{
    ...
    printk("[AE9] RXFIIQ calibration enhancement CL141424 20151118.\n");
...
char *GetApCliStatusStr(RTMP_ADAPTER *pAd)
{
        POS_COOKIE pObj;
        UCHAR ifIndex;
        ...
        if (pObj->ioctl_if_type != INT_APCLI)
        {
                sprintf(msg,"Not APCLI interface");
                return msg;
        }
                ...
                for (i=0; i<MAX_LEN_OF_MAC_TABLE; i++)
                {
                                ...
                                && (pEntry->Sst == SST_ASSOC))
                        {
                                if(pEntry->PortSecured == WPA_802_1X_PORT_SECURED)
                                {
                                        ...
                                        bConnect=TRUE;
                                        sprintf(msg,"Connected,%d", RTMPAvgRssi(pAd,&pEntry->RssiSample));
                                        break;
                                }
                                else
                                {
                                        bConnect=TRUE;
                                        sprintf(msg,"NotSecured,%d", RTMPAvgRssi(pAd,&pEntry->RssiSample));
                                        break;
                                }
                        }
                }
...
#ifdef EFM_MISC_PATCH
    sprintf(msg+strlen(msg), "Ap Client Status=%s\n", GetApCliStatusStr(pAd));
#endif
	...
	IN	PSTRING			arg)
{
	...
	if ((strcmp(arg, "WEPAUTO") == 0) || (strcmp(arg, "wepauto") == 0))
        ...
        {
                pAd->ApCfg.ApCliTab[ifIndex].AuthMode = Ndis802_11AuthModeWPA1PSKWPA2PSK;
...
========================================================================
*/
#ifdef EFM_MISC_PATCH_XXX
VOID RTMPIoctlGetApcliStatus( IN      VOID   *pAdapter, IN      RTMP_IOCTL_INPUT_STRUCT   *wrq);
#endif
	...
	IN	ULONG					Data)
{
	...
	switch(cmd)
	{
		...
		case CMD_RTPRIV_IOCTL_STATISTICS:
			...
			break;
#ifdef EFM_MISC_PATCH_XXX
                case CMD_RTPRIV_IOCTL_APCLISTATUS:
                        RTMPIoctlGetApcliStatus(pAd, wrq);
                        break;
#endif
#ifdef WSC_AP_SUPPORT
