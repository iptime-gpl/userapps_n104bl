	...
	IN	INT			ifIndex)
{
	...
	DBGPRINT(RT_DEBUG_OFF, (" ApCliRTMPReportMicError <---\n"));
	...
	{
		DBGPRINT(RT_DEBUG_OFF, (" MIC Error count = 2 Trigger Block timer....\n"));
		DBGPRINT(RT_DEBUG_OFF, (" pAd->ApCfg.ApCliTab[%d].LastMicErrorTime = %ld\n",ifIndex,
    ...
    IN PVOID SystemSpecific3) 
{
	...
	DBGPRINT(RT_DEBUG_OFF, ("(%s) disassociate with current AP after sending second continuous EAPOL frame.\n", __FUNCTION__));
