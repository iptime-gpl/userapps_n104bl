	...
	if(Status == MLME_SUCCESS)
	{
		...
		if (ApCliLinkUp(pAd, ifIndex))
		{
			...
			*pCurrState = APCLI_CTRL_CONNECTED;
			//
			// This fine tuning of TXOP_BE is required for
			// TGac 5.2.32 and TGn 5.2.32 as very less packets
			// get in queued for TX.
			// These SSIDs are defined in the document.
			//
			if(NdisEqualMemory(pAd->ApCfg.ApCliTab[ifIndex].CfgSsid, "WEI823>", pAd->ApCfg.ApCliTab[ifIndex].CfgSsidLen) ||
			   NdisEqualMemory(pAd->ApCfg.ApCliTab[ifIndex].CfgSsid, "VHT-5.2.32", pAd->ApCfg.ApCliTab[ifIndex].CfgSsidLen)) {
				DBGPRINT(RT_DEBUG_TRACE, ("(%s) CfgSsid =%s CfgSsidLen = %d is matching to WEI823> or VHT-5.2.32\n", __FUNCTION__,
											pAd->ApCfg.ApCliTab[ifIndex].CfgSsid,pAd->ApCfg.ApCliTab[ifIndex].CfgSsidLen ));
				RTMP_IO_READ32(pAd, 0x0220, &r220);
            	r220 |= 0x100;
            	RTMP_IO_WRITE32(pAd, 0x0220, r220);
			}else {
				RTMP_IO_READ32(pAd, 0x0220, &r220);
				//
				// This is to unset the set value in 0x220 register
				// once TC 5.2.32 ran successfuly and SQA going to run any other test.
				//
				if(r220 == 0x100) {
					RTMP_IO_WRITE32(pAd, 0x0220, 0);
				}
				DBGPRINT(RT_DEBUG_TRACE, ("(%s) CfgSsid =%s CfgSsidLen = %d is not matching to WEI823> or VHT-5.2.32\n", __FUNCTION__,
					pAd->ApCfg.ApCliTab[ifIndex].CfgSsid,pAd->ApCfg.ApCliTab[ifIndex].CfgSsidLen ));
			}
		}
		else
