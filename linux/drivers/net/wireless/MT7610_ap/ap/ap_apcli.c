	...
	IN UCHAR ifIndex)
{
	...
	*/
    ...
    {
		DBGPRINT(RT_DEBUG_TRACE, ("WSCSTOP: %d, %d, %d\n", result, pAd->ApCfg.ApCliTab[ifIndex].WscControl.WscConfMode, 
	...
	IN	RTMP_NET_PACKET_TRANSMIT	Func)
{
	...
	for(apcliIndex = 0; apcliIndex < MAX_APCLI_NUM; apcliIndex++)
	{
		...
		if (pApCli[apcliIndex].dev == dev_p)
		{
			...
			NdisZeroMemory((PUCHAR)(GET_OS_PKT_CB(skb_p)+CB_OFF),15);
	...
	IN  PRTMP_ADAPTER   pAd)
{
	...
	*/
	for (entryIdx=0; entryIdx<pScanTab->BssNr;entryIdx++)
	{
        ...
        /* From rlt driver : for WPAPSK/WPA2PSK mixed configuration apcli */
            if (pAd->ApCfg.ApCliTab[ifIdx].bMixAuthMode == TRUE)
            {
                if (pBssEntry->WPA2.PairCipher != Ndis802_11WEPDisabled)
                {
                    pAd->ApCfg.ApCliTab[ifIdx].wdev.AuthMode =  pBssEntry->AuthMode;
                    pAd->ApCfg.ApCliTab[ifIdx].wdev.WepStatus = pBssEntry->WPA2.PairCipher;
                }
                else if (pBssEntry->WPA.PairCipher != Ndis802_11WEPDisabled)
                {
                    pAd->ApCfg.ApCliTab[ifIdx].wdev.AuthMode =  pBssEntry->AuthMode;
                    pAd->ApCfg.ApCliTab[ifIdx].wdev.WepStatus = pBssEntry->WPA.PairCipher;
                }
                else
                {
                }
